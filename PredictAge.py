...
for file_path in csv_files_folder1:
    ...
    for i in range(samples.shape[1]):
        axs[i].plot(freq_positive, abs(fft_magnitude[:len(freq_positive), i]))
...
# Giới hạn các dự đoán trong khoảng từ 2 đến 78
predictions = np.clip(predictions, 2, 78)

for true_age, prediction in zip(test_labels, predictions):
    print(f"True Age: {true_age}, Predicted Age: {int(prediction)}")
