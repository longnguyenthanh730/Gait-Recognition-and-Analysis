...
# Trích xuất ID dạng số từ tên tệp tin
def extract_numeric_id(file_name):
    return int(file_name.split("_")[1].split(".")[0][2:])
...
for file_path in csv_files_folder1:
    ...
    freq_positive = freq[np.where((freq >= 0) & (freq <= 0.3))]
    ...
    # Lấy 150 giá trị đã qua biến đổi Fourier
    fft_samples_150 = fft_magnitude[:150]

    # Tạo DataFrame từ mảng numpy
    df_150 = pd.DataFrame(fft_samples_150, columns=["Gx", "Gy", "Gz", "Ax", "Ay", "Az"])
    ...
    df_150.to_csv(new_file_path, index=False)
...
for file_path in glob.glob('Processed_Data_Train/*.csv'):
    ...
    file_name = Path(file_path).name
    # Trích xuất mã ID từ tên file và convert thành int
    file_id = extract_numeric_id(file_name)
    train_labels.append(file_id)
...
for file, prediction in zip(test_labels, predictions):
    print(f"File: {file}")
